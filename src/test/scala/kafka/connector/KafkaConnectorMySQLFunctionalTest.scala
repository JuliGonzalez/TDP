...
class KafkaConnectorMySQLFunctionalTest extends WordSpec with Matchers with BeforeAndAfterAll {
  var db: Connection = _
  ...
  override def beforeAll(): Unit = {
    ...
    publisherTimestamp             = kafka.consume(ConsumerProperties("localhost:9092", "test-timestamp-mysql", "functional-test-mysql", deserializer, deserializer))
    publisherIncrementing          = kafka.consume(ConsumerProperties("localhost:9092", "test-incrementing-mysql", "functional-test-mysql", deserializer, deserializer))
    publisherTimestampIncrementing = kafka.consume(ConsumerProperties("localhost:9092", "test-timestamp-incrementing-mysql", "functional-test-mysql", deserializer, deserializer))
  }
