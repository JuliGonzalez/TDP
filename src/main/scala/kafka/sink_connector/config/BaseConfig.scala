...
abstract class BaseConfig(props: Map[String, String]) {
   ...
   */
  def connectionUser: String = props("connection.user")

  /**
   * DB Jdbc password for source & sink
   */
  def connectionPassword: String = props("connection.password")

  /**
   * List of topics to be published or subscribed by source or sink
   */
  def topics: List[String] = props("topics").toString.split(",").toList

  /**
   * Batch size for sink. Should be an integer.
   * Default is 3000.
   */
  def batchSize: Int = props.getOrElse[String]("batch.size", "3000").toInt

  /**
   * Max retries for sink. Should be an integer.
   * Default is 10.
   */
  def maxRetries: Int = props.getOrElse[String]("max.retries", "1").toInt

  /**
   * Auto create HANA tables if table is not found for Sink.
   * Default is false.
   */
  def autoCreate: Boolean = props.getOrElse[String]("auto.create", "false").toBoolean

  /**
   * Max rows to include in a single batch call for source. Should be an integer.
   * Default is 100.
   */
  def batchMaxRows: Int = props.getOrElse[String]("batch.max.rows", "100").toInt

  /**
   * Query mode for polling data from table for source.
   */
  def queryMode: String = props.getOrElse[String]("queryMode", BaseConfigConstants.QUERY_MODE_TABLE)

  /**
   * The mode for updating a table each time it is polled.
   * Default is 'bulk'.
   */
  def mode: String = props.getOrElse[String]("mode", BaseConfigConstants.MODE_BULK)

  /**
   * Whether to auto update Schema from Database for every record flush for sink
   * Default is 'false'.
   */
  def autoSchemaUpdateOn: Boolean = props.getOrElse[String]("auto.schema.update", "false").toBoolean
