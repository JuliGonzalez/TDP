...
case class PersonalJdbcClient(personalConfiguration: PersonalConfig)  {
      ...
      new PersonalJdbcConnectionException("Cannot acquire a connection")) { () =>
       ...
       match {
         ...
         case Failure(ex) if isFailedToOpenConnectionException(ex) =>
           throw new PersonalJdbcConnectionException(s"Opening a connection failed")
         case Failure(ex) =>
           /* Make a distinction between bad state and network failure */
           throw new PersonalJdbcBadStateException("Cannot acquire a connection")
       }
     }
  }
  ...
  def createTable(namespace: Option[String],
    ...
    ExecuteWithExceptions[Unit, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"Creation of table $tableName failed")) { () =>
      val testSchema = PersonalSchemaBuilder.avroToHANASchema(tableSchema)
      val fullTableName = tableWithNamespace(namespace, tableName)
      ...
      if (partitionType == BaseConfigConstants.HASH_PARTITION) {
        if (keys == null) {
          throw new PersonalJdbcException("Hash Partition is not supported when keys are" +
  ...
  def createCollection(collectionName: String): Unit = {
    ExecuteWithExceptions[Unit, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"Creation of collection $collectionName failed")) { () =>
  ...
  def tableExists(namespace: Option[String], tableName: String): Boolean =
    ExecuteWithExceptions[Boolean, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"Checking if table $tableName exists in vora backend failed")) { () =>
      WithCloseables(getConnection) { conn =>
  ...
  def collectionExists(collectionName: String): Boolean =
    ExecuteWithExceptions[Boolean, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"Checking if collection $collectionName exists in hana backend failed")) { () =>
      WithCloseables(getConnection) { conn =>
        try {
  ...
  def getExistingTables(dbschemaPattern: String, tableNamePattern: String): Try[Seq[String]] =
    ExecuteWithExceptions[Try[Seq[String]], Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"Retrieving exisiting VORA tables failed matching schema $dbschemaPattern and table $tableNamePattern failed")) { () =>
      WithCloseables(getConnection) { conn =>
        WithCloseables(conn.createStatement()) { stmt =>
          Try {
  ...
  def getExistingSchemas(dbschemaPattern: String, tableNamePattern: String): Seq[ColumnRow] =
    ExecuteWithExceptions[Seq[ColumnRow], Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"retrieving schema for database $dbschemaPattern and table name $tableNamePattern failed")) { () =>
      WithCloseables(getConnection) { conn =>
        WithCloseables(conn.createStatement()) { stmt =>
  ...
  def dropTable(namespace: Option[String], tableName: String): Unit =
    ExecuteWithExceptions[Unit, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"drop table $tableName is not successful")) { () =>
      WithCloseables(getConnection) { conn =>
        WithCloseables(conn.createStatement()) { stmt =>
          val fullTableName = tableWithNamespace(namespace, tableName)
  ...
  def getQueryStatistics(tableNameOrSubquery: String): Int = {
    ExecuteWithExceptions[Int, Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"cannot fetch query statistics for table or subquery $tableNameOrSubquery")) { () =>
      WithCloseables(getConnection) { conn =>
        WithCloseables(conn.createStatement()) { stmt =>
  ...
  def executeQuery(schema: Schema, queryString: String, offset: Int, limit: Int,
    ...
    ExecuteWithExceptions[Option[List[Struct]], Exception, PersonalJdbcException] (
      new PersonalJdbcException(s"execution of query $queryString failed")) { () =>
      WithCloseables(getConnection) { conn =>
        WithCloseables(conn.createStatement()) { stmt =>
          if (limit > 0 && offset > 0) {
  ...
  def commit(connection: Connection): Unit = {
    ExecuteWithExceptions[Unit, Exception, PersonalJdbcConnectionException] (
      new PersonalJdbcConnectionException("commit failed for current connection")) { () =>
        connection.commit()
    }
  }
